...
# Removed because in solaris sparc does not have
...
up() {
	...
	if [ -f $JBOSS_PIDFILE ]; then
        ...
        ppid_confirmation=$(su - $JBOSS_USER -c "pgrep -u $JBOSS_USER | grep $ppid")
        if [ ! -z $ppid_confirmation  ]; then
	...
	do
		...
		if [ -f $JBOSS_MARKERFILE ]; then
			markerfiletimestamp=$(awk '{ split($0,a,":"); print a[2]}' $JBOSS_MARKERFILE) > /dev/null
            if [ "$markerfiletimestamp" -gt "$currenttime" ] ; then
				grep 'success:' $JBOSS_MARKERFILE > /dev/null
				if [ $? -eq 0 ] ; then
                    launched=true
					break
			    fi
...
down() {
    ...
    if [ -f $JBOSS_PIDFILE ]; then
        ...
        # Try issuing SIGTERM
        su - $JBOSS_USER -c "kill -15 $kpid"

        until [ ! -z su - $JBOSS_USER -c "pgrep -u $JBOSS_USER | grep $ppid" ]
        do
            sleep 1
            let count=$count+1;
        ...
        if [ $count -gt $kwait ]; then
            su - $JBOSS_USER -c "kill -9 $kpid" 2> /dev/null
            if [ "$?" != 0  ]; then
                failure
                 exit 1
            fi
